...
const authConfig = {
    ...
    version: "2.1.1-DPlayer", // Custom version
...
/**
 * Renders the dedicated file view/player page using DPlayer.
 * @param {number} driveId The current drive ID.
...
async function renderFilePageHTML(driveId, file, url) {
    ...
    let playerContainer = '';
    if (isVideo || isAudio) {
        // DPlayer will be rendered in this div
        playerContainer = `<div class="player-container"><div id="dplayer"></div></div>`;
    } else if (isImage) {
        // Standard image view
        playerContainer = `<div class="player-container"><img src="${directViewUrl}" alt="${fileName}" style="max-width: 100%; max-height: 80vh; display: block; margin: auto; border-radius: 8px;"></div>`;
    }

    // The entire HTML page for file viewing
    return `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <link rel="icon" href="/favicon.ico">
    ${isVideo || isAudio ? '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.css">' : ''}
    <style>
        ...
        .player-container { margin-bottom: 2rem; background-color: #000; border-radius: 8px; }
        #dplayer { border-radius: 8px; }
        ...
        .toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background-color: rgba(0,0,0,0.7); color: white; padding: 10px 20px; border-radius: 5px; z-index: 1000; opacity: 0; transition: opacity 0.5s; pointer-events: none; }
...
<body>
    ...
    <script>
        try {
            ...
            console.error("DPlayer initialization failed:", e);
            const playerContainer = document.getElementById('dplayer');
            if (playerContainer) {
                playerContainer.style.color = 'white';
                playerContainer.style.padding = '2rem';
                playerContainer.innerText = '播放器加载失败，请尝试刷新或直接下载文件。';
            }
        }
    </script>
    ` : ''}
    ...
    <script>
        ...
        function copyToClipboard(text) {
            // Using a textarea element for broader compatibility
